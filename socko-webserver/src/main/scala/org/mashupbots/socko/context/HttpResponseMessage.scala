 ...
 *
 * @param context Context associated with this request
 */
case class HttpResponseMessage(context: HttpContext) {

  /**
   * Request containing info for connection management
   */
  private val request: HttpRequestMessage = context match {
    case ctx: HttpRequestContext => ctx.request
    case ctx: HttpChunkContext => ctx.initialHttpRequest
  }
  ...
  def write(content: Array[Byte]): Unit = {
    ...
    // Write web log
    context.writeWebLog(response.getStatus.getCode, response.getContent.readableBytes)

    // Write the response.
    val future = context.channel.write(response)
  ...
  private def setContent(response: HttpResponse, content: Array[Byte], contentType: String) {
    ...
    val compressible = (content != null &&
      content.size > 0 &&
      content.size >= context.config.minCompressibleContentSizeInBytes &&
      content.size <= context.config.maxCompressibleContentSizeInBytes &&
      context.config.compressibleContentTypes.exists(s => contentType.startsWith(s)))
  ...
  def writeLastChunk(trailingHeaders: Map[String, String] = Map.empty[String, String]): Unit = {
    ...
    val future = context.channel.write(lastChunk)
    if (!request.isKeepAlive) {
      future.addListener(ChannelFutureListener.CLOSE)
    }

    context.writeWebLog(this.status.code, totalChunkContentLength)
    hasBeenWritten = true
  }
...
object HttpResponseMessage {
  ...
  def setDateHeader(response: HttpResponse) {
    ...
    dateFormatter.setTimeZone(TimeZone.getTimeZone(HttpContext.HTTP_DATE_GMT_TIMEZONE))
